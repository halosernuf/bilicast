...
var CastPlayer = function() {
  ...
  // @type {string} Current media title
 ...
 */
CastPlayer.prototype.loadMedia = function(mediaUrl, contentType, title, img) {
  if (!this.session) {
...
CastPlayer.prototype.onMediaDiscovered = function(how, mediaSession) {
  ...
  if( how == 'activeSession' ) {
    ...
    this.currentMediaTitle = this.session.media[0].media.metadata.title;
    if ($('#url').val() == "") {
      $('#url').val(this.currentMediaUrl);
    }
    $('#player_now_playing').html(this.currentMediaTitle);
    // $('#controls').show();
  }
 ...
 */
CastPlayer.prototype.seekMedia = function(is_forward) {
